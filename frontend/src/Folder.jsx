...
const Folder = ({ color = '#5227FF', size = 1, items = [], className = '', onPaperClick }) => {
  ...
  return (
    <div style={scaleStyle} className={className}>
      <div
        className={`folder-group ${!open ? 'folder-closed' : ''}`}
        style={{
          ...folderStyle,
      ...
      >
        ...
        >
          {papers.map((item, i) => {
            ...
            if (i === 1) sizeStyle = open ? { width: '80%', height: '80%' } : { width: '80%', height: '70%' };
            if (i === 2) sizeStyle = open ? { width: '90%', height: '80%' } : { width: '90%', height: '60%' };
          ...
          <div
            className={`folder-front folder-front-1 ${!open ? 'folder-front-closed-1' : ''}`}
            style={{
          ...
          <div
            className={`folder-front folder-front-2 ${!open ? 'folder-front-closed-2' : ''}`}
            style={{
              backgroundColor: color,
              ...(open && { transform: 'skew(-15deg) scaleY(0.6)' })
            }}
